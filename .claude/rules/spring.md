...
Use the `spring` CLI (Spring Boot CLI) for project init/scaffolding when available — check with `command -v spring`. Fall back to Spring Initializr (start.spring.io) only if the CLI isn't installed.
